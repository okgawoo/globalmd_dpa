...
- **그린 계열 색상 전면 금지** — `#10B981`, `#1D9E75` 등 초록 계열 절대 사용 금지 (상태 표시 포함)
...
| **구분선 레이블** | `12px` | `700` | 설계사 전용 등 섹션 타이틀 |

### ⚠️ 폰트 사이즈 최솟값 규칙

- **절대 최솟값: 10px** — 배지에만 허용
- **본문/레이블 최솟값: 12px** — 12px 미만은 일반 텍스트에 사용 금지
- Claude는 기본적으로 폰트를 작게 설정하는 경향이 있어 반드시 의식적으로 체크할 것
- 새 컴포넌트 작성 시: 디폴트 14px 기준으로 시작, 필요 시 조정

### 한국어 디자인 용어 (오빠와 대화 시 기준)

| 용어 | 의미 | 예시 |
|---|---|---|
| **흐리게** | 색을 회색 방향으로 이동 (채도↓, 명도 조정) | `#636B78` → `#8892A0` |
| **진하게** | 색을 더 짙게 (채도↑, 명도↓) | `#636B78` → `#1A1A2E` |
| **얇게** | font-weight를 낮춤 | `font-weight: 700` → `500` |
| **굵게** | font-weight를 높임 | `font-weight: 400` → `600` |
| **밝게** | 색의 밝기(명도)를 높임 — 흐리게와 다름 | 연하게 ≒ 밝게 |
...
| 사이드바 축소 너비 | `60px` |
| 헤더 높이 | `52px` |
| 페이지 하단 여백 (padding-bottom) | `96px` |

> **96px 기준 이유**: FAB(`bottom: 28` + `height: 52` = 상단 80px)보다 위에 콘텐츠가 가려지지 않도록 + 사이드바 하단 계정카드와 시각적 정렬.  
> 적용 위치: 각 페이지 `.page` wrapper 또는 스크롤 컨테이너의 `padding-bottom`.  
> **예외**: 고객 관리 등 3열 고정 레이아웃처럼 내부 스크롤 구조가 별도인 페이지는 리스트 컨테이너에 `margin-bottom: 96px`으로 대체 적용.
...
## 차트 컬러 팔레트 (Recharts 기준)

> 아래 색상은 Google Sheets 템플릿 레퍼런스에서 추출. 세련된 무채색+어스톤 기준.

### 상태별 막대 색상 (STATUS_COLOR)

| 상태 | 색상 | Hex | 용도 |
|---|---|---|---|
| good (충분) | 스틸 블루 | `#2E5F8A` | 기준 달성 항목 |
| ok (보통) | 번트 오렌지 | `#C5621C` | 부족하지 않은 항목 |
| low (부족) | 딥 레드 | `#9B2335` | 보장 공백 항목 |

### 상태별 배경색 (STATUS_BG)

| 상태 | Hex | 용도 |
|---|---|---|
| good | `#E8F0F7` | 막대/카드 배경 연한 파랑 |
| ok | `#FBF0E8` | 막대/카드 배경 연한 피치 |
| low | `#F7E8EA` | 막대/카드 배경 연한 레드 |

### 도넛/파이 차트 팔레트 (PIE_COLORS)

```
['#5E6AD2', '#2A7D8D', '#C5622D', '#7C1E5E', '#1E54B8', '#3A2E8A', '#D4945A', '#94A3B8']
```

| 인덱스 | Hex | 색상명 |
|---|---|---|
| 0 | `#5E6AD2` | 인디고 (accent 기본) |
| 1 | `#2A7D8D` | 틸 |
| 2 | `#C5622D` | 번트 오렌지 |
| 3 | `#7C1E5E` | 딥 플럼 |
| 4 | `#1E54B8` | 로얄 블루 |
...
**금지**: 초록 계열 (#10B981, #1D9E75 등) 차트에서도 절대 사용 금지
...
*Source: `styles/admin.css`, `Layout.module.css`, `Dashboard.module.css`, `Customers.module.css`, `Report.module.css` / 마지막 업데이트: 2026-04-30*

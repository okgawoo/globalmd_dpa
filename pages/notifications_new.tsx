...
export default function NotificationsPage() {
  ...
  async function fetchAll() {
    ...
    if (aid) {
      try {
        const res = await fetch(`/api/sms/usage?agent_id=${aid}`)
        if (res.ok) { const data = await res.json(); setSmsUsage(data) }
      } catch (e) { console.error('SMS 사용량 조회 실패:', e) }
    }
  }
  ...
  async function handleSend() {
    ...
    if (smsUsage && smsUsage.remaining <= 0) { alert('이번 달 문자 발송 한도를 초과했습니다.'); return }
    if (!selected.customer.phone) { alert('고객 연락처가 없습니다.'); return }
    setSending(true)
    try {
      const res = await fetch('/api/sms/send', {
        ...
        body: JSON.stringify({
          ...
          message_type: selected.type || selected.notifType,
        }),
      })
      const result = await res.json()
      if (result.success) {
        setSmsUsage(prev => prev ? { ...prev, used: result.usage.used, remaining: result.usage.remaining } : prev)
        await fetchAll()
        alert(`${selected.customer.name} 님께 문자가 발송됐어요! 😊 (남은 ${result.usage.remaining}건)`)
      } else {
      ...
      <div className={styles.pcGrid}>
        ...
        <div className={styles.phoneCol}>
            ...
            <>
              ...
              {smsUsage && (
                <div style={{
                  ...
                  backgroundColor: smsUsage.remaining <= 10 ? '#FFF3F3' : '#F0FAF5',
                  border: `1px solid ${smsUsage.remaining <= 10 ? '#FFD4D4' : '#D4F0E4'}`,
                  fontSize: '13px',
                }}>
                  <span style={{ color: '#666' }}>📨 이번 달 문자</span>
                  <span style={{ fontWeight: 700, color: smsUsage.remaining <= 10 ? '#E53E3E' : '#1D9E75' }}>
                    {smsUsage.remaining} / {smsUsage.limit}건 남음
                  </span>
                </div>
              )}
          ...
          <div className={styles.actionBtns}>
            <button className={styles.btnSend} onClick={handleSend} disabled={sending || !selected || (smsUsage !== null && smsUsage.remaining <= 0)}>{sending ? '발송 중...' : (smsUsage && smsUsage.remaining <= 0) ? '한도 초과' : '발송하기'}</button>

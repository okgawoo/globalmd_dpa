...
export default function Sales() {
  ...
  useEffect(() => {
    ...
    if (tab === 'meeting') setActiveTab('meeting')
    if (meetingId) {
  ...
  return (
    <div className={styles.wrap}>
      ...
      <div className={styles.tabs}>
        ...
        <button className={[styles.tab, activeTab === 'meeting' ? styles.active : ''].join(' ')} onClick={() => setActiveTab('meeting')}>📅 미팅 일정</button>
      ...
      {activeTab === 'meeting' && (
        <div style={{paddingTop:12}}>
          ...
          {/* 하위 탭 */}
          <div style={{display:'flex',gap:6,marginBottom:16,borderBottom:'1px solid #F3F4F6',paddingBottom:8}}>
            {(['today','week','next','past'] as const).map(t => (
              <button key={t}
                onClick={() => setMeetingSubTab(t)}
                style={{
                  ...
                  {upcomingMeetings.map(m => {
                    ...
                    return (
                      <div key={m.id} className={styles.meetingCard}>
                        ...
                        <div style={{fontSize:12,color:'#6B7280',marginTop:2}}>📅 {dateLabel} {m.meeting_time||''}</div>
                        <div className={styles.meetingBottom}>
                          <span className={styles.meetingLocation}>📍 {m.location || '장소 미정'}</span>
                          <span className={styles.typeBadge}>{m.type || '미팅'}</span>
                          {m.cancel_count > 0 && <span className={styles.cancelWarn}>⚠ 취소 {m.cancel_count}회</span>}
          ...
          {meetingSubTab === 'next' && (
            <div>
              {nextWeekDays.map(day => {
                const dayMeetings = nextWeekMeetings.filter(m => m.meeting_date === day)
                const dateObj = new Date(day + 'T00:00:00')
                const dayLabel = ['일','월','화','수','목','금','토'][dateObj.getDay()]
                const monthDay = `${dateObj.getMonth()+1}/${dateObj.getDate()}`
                return (
      ...
      {activeTab === 'contact' && (
        <div style={{paddingTop:16}}>
          ...
          <div className={styles.sectionHeader} style={{marginBottom:8}}>
            ...
            <span style={{fontSize:11,color:'#9CA3AF',padding:'4px 8px'}}>AI 자동 추출</span>
          </div>

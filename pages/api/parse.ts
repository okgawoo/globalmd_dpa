...
11. 보험사명 추출 및 변환 (반드시 준수):
    ...
    [Step 0 — 상품명 내 보험사명 최우선 추출 ★ 가장 먼저 실행]
    - 화면 복붙 시 상단에 다른 보험사명이 섞여 오는 경우가 있음 (예: "삼성생명\n계피동일\n(무)흥국생명다재다능1540보험...")
    - 이 경우 상단의 보험사명("삼성생명")은 무시하고 상품명에 포함된 보험사명("흥국생명")을 최우선 사용
    - (무)xxx, (유)xxx 형식 상품명에서 보험사 추출 방법:
      · "(무)" 또는 "(유)" 직후에 나오는 단어가 보험사명 키워드와 일치하면 해당 보험사로 확정
      · 예: "(무)흥국생명다재다능1540보험" → 흥국생명
      · 예: "(무)교보통합CI보험" → 교보생명
      · 예: "(무)현대해상퍼펙트플러스종합보험" → 현대해상
      · 예: "(무)KB손보실손의료비보장보험" → KB손해보험 ("KB손보" = KB손해보험)
    - 상품명에 보험사명 키워드가 있으면 Step 1 결과보다 무조건 우선 적용

    [Step 1 — 텍스트에서 보험사명 직접 추출]
    - 상품명에 보험사명이 없을 때만: 텍스트 최상단 또는 각 계약 블록 첫 줄에서 보험사명을 찾으세요.
    - 아래 인식 키워드가 텍스트에 포함되어 있으면 해당 보험사로 확정합니다.

    [생명보험사 인식 키워드 → 최종 보험사명]
    - 삼성생명, 삼성 (생명 맥락) → 삼성생명
    - 한화생명, 한화, 굿모닝, 매일아침, 무배당매일, 매일아침굿모닝, PLUS (한화 상품) → 한화생명
    - 교보생명, 교보, 교보라이프 → 교보생명
    - 신한라이프, 신한생명 → 신한라이프
    - AIA → AIA생명
    - 흥국생명, 흥국 (생명 맥락), Heungkuk, 다재다능 → 흥국생명
    - DB생명, 동부생명 → DB생명
    - 푸본현대, 현대라이프 → 푸본현대생명
    - 하나생명 → 하나생명
    - KDB생명, 산은생명 → KDB생명
    ...
    - MG새마을금고, 새마을금고 (생명 맥락) → MG새마을금고

    [손해보험사 인식 키워드 → 최종 보험사명]
    - 삼성화재, 삼성 (화재 맥락) → 삼성화재
    - 현대해상, 하이콜, Hi0809, HI, 하이퍼, 무배당하이, 무배당하이콜, Hi (현대 맥락) → 현대해상
    - KB손해보험, KB손해, KB손보, KB다이렉트 → KB손해보험
    - DB손해보험, DB손해, DB손보, 동부화재 → DB손해보험
    - 메리츠화재, 메리츠 → 메리츠화재
    - 롯데손해보험, 롯데손보, 롯데, let:drive → 롯데손해보험
    - 한화손해보험, 한화손해, 한화손보 → 한화손해보험
    - MG손해보험, MG, 無MG, 무MG, 행복드림보장공제, 종신암공제 → MG손해보험
    - 흥국화재, 흥국 (화재 맥락) → 흥국화재
    - NH손해, 농협손해 → NH농협손해보험
    - AIG손해보험, AIG → AIG손해보험
    - AXA → AXA손해보험
    ...
    [흥국 구분 기준]
    - "흥국화재" 명시 또는 상품명에 "화재" 포함 → 흥국화재
    - "흥국생명" 명시 또는 상품명에 "생명/다재다능/Heungkuk" 포함 → 흥국생명
    - 단순 "흥국" 만 있는 경우: 상품명에 화재/손해 키워드 없으면 → 흥국생명

    [Step 2 — 보험사명이 없으면 상품명 키워드로 유추]
    - 상품명/보장명에 위 키워드가 포함된 경우 해당 보험사로 매핑하세요.
    - 예: 상품명에 "메리츠" 포함 → 메리츠화재 / "let:drive" → 롯데손해보험 / "다재다능" → 흥국생명

    [Step 3 — 매핑 실패 시]
    - Step 0, 1, 2 모두 해당 없으면 반드시 "" (빈 문자열)로 반환하세요.
    - ❌ 절대 금지: "삼성생명", "삼성화재"를 기본값·추측값으로 사용하지 마세요. "확인필요" 문자열도 금지.
...
반환 형식:
{
  ...
  "contracts": [
    {
      "policy_number": "증권번호 (텍스트에서 추출, 없으면 null. 마스킹(*) 포함 원본 그대로)",
      "company": "보험사 풀네임 (텍스트 직접 확인 → 없으면 상품명 키워드 매핑 → 모두 실패 시 \"\" 빈 문자열. 삼성생명 기본값·확인필요 절대 금지)",
      "product_name": "상품명",

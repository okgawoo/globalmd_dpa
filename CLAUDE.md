...
## ⚠️ 3대 절대 금지 규칙 (위반 시 작업 데이터 유실)

> **반드시** 워크트리(`.claude/worktrees/`) 생성/수정 절대 금지
> **반드시** 모든 작업은 메인 프로젝트 경로에서만
> **반드시** 채팅방 이동 전 commit 필수
...
## ⚠️ 인증 키 / 환경변수 절대 원칙 (2026-05-01 사고 후 추가)

> VAPID 키 교체 사고: 잘 작동하던 푸시 구독 19개 전체 무효화. 수백 명 서비스였으면 대형 사고.

1. **작동 중인 키/환경변수는 절대 건드리지 않는다**
   - 오류 발생 시 → 오류 로그·코드 먼저 확인 → 원인 특정 후 최소 범위만 수정
   - "Warning", "Needs Attention" 배지 = 내용 먼저 읽고 판단 (즉시 교체 금지)

2. **VAPID 키는 교체 = 모든 push 구독 무효화**
   - 한번 교체하면 기존 브라우저 구독 전부 재구독 필요
   - 교체 전 반드시 오빠 확인 필수

3. **DB 레코드 삭제·환경변수 변경은 사전 설명 + 허락 후 진행**
   - "복구해봐" = 삭제 허락이 아님
   - 되돌릴 수 없는 작업은 반드시 먼저 설명하고 명시적 허락 받을 것
